...
Studentprofile.getStudentModel = (studentprofileReqData,result) =>{
    ...
    dbConn.query(`SELECT * FROM student_admissions LEFT JOIN grade_master ON student_admissions.grade_id = grade_master.grade_master_id LEFT JOIN grade_section ON student_admissions.grade_section_id = grade_section.grade_section_id where student_admissions.student_admissions_id = "${admission_id}";`,(err,res)=>{
       if(res){
            result(null,res);
       }else{
            result(null,err);
