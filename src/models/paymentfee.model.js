...
Payment.getPaymentModel = (paymentReq, result) => {
    ...
    dbConn.query(`SELECT * from student_payment_infos where student_id = "${paymentReq.student_id}" and year_id ="${paymentReq.year_id}" and term_name ="${paymentReq.term_name}" and optional_fees=false;`,(err, res) =>{
        if(res){
            let temArr = [];
            let pusharr = [];
            let balance = 0;
